...
// this is just for sorting characters downwards by their commonity 
void frequencySort(int *A, int len, int *freq) {
  if (len < 2) return;
  ...
  frequencySort(A, i, freq);
  frequencySort(A + i, len - i, freq);
}
...
//int alphabet[8] = { 4, 3, 4, 3, 4, 4, 3, 3 }; // <-- IMPORTANT, SET THE RIGHT VALUES !!!
...
/*
     ...
     THIS DOES ONLY WORKS IN PYTHON
...
alphabet[p] = mlen;
frequencySort(map,mlen,frequencies);
...
//printf("parallel execution time FIRST:  %f ms\n",parallel_time);
