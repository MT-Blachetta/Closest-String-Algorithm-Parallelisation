"""Sequential evolutionary solver for the closest string problem.

The script searches for a string that minimises the maximum Hamming
distance to a given list of input strings.  This version operates on
strings of variable length without any parallelisation.
"""
...
def count_all(duplicates, ratio=False, rounded=0):
    ...
    Behaves like ``closestStringEvoMPI.count_all`` and can optionally
...
def mutate(arg,map_len):
    """Mutate a candidate by randomly changing one character."""
    random.seed(None)
...
@profile
def execution():
        """Run the evolutionary search in a single process."""
        ...
        for g in range(generation):
            ...
            # print('Best candidate of the generation: '+ str(winner) )
